...
class CheckoutController extends Controller
{
     ...
     */
    public function postCheckout($infoCheckout)
    {
        $infoCheckout = json_decode($infoCheckout);
        $receiver = RECEIVER;
        //Mã đơn hàng
        $order_code = $infoCheckout->order_code;
        //Khai báo url trả về
        $return_url = route('checkoutNl.success');
        // Link nut hủy đơn hàng
        $cancel_url = env('APP_URL');
        //Giá của cả giỏ hàng
        $txh_pageName = $infoCheckout->txh_pageName;
        $txh_username = 'Active package ' . $infoCheckout->package . ' by: ' . $infoCheckout->txh_username . ', ID Facebook: ' . $infoCheckout->txh_uid;
        $txh_pageId = $infoCheckout->txh_pageId . '@facebook.com';
        $price = $infoCheckout->price;
        //Thông tin giao dịch
        $transaction_info = $infoCheckout->package . "*|*" . $infoCheckout->package_id . "*|*" . $infoCheckout->price . "*|*" . $infoCheckout->month;
        ...
        $fee_cal = 0;
        $fee_shipping = 0;
        $order_description = "Package: " . $infoCheckout->package;

        // Sửa thông tin người mua hàng tại đây
        $buyer_info = $txh_pageName . "*|*" . $txh_pageId . "*|*" . '0000000000' . "*|*" . $txh_username;
        $affiliate_code = "";
    ...
    public function success(Request $request)
    {
        ...
        if ($checkpay) {
            ...
            if ($checkResult !== false) {
                $transaction = Transaction::where(explode('_', $order_code)[1])->first();
                if ($transaction != null && $checkResult->error_code == "00" && (!isset($transaction->token) || $transaction->token != $request->token)) {
                    ...
                    $transaction->save();
                    $newPageActive = new PageActive();
                    $newPageActive->page_id = $transaction->page_id;
                    $newPageActive->upgrade = $transaction->package_id;
                    $newPageActive->start_upgrade = Carbon::now();
                    $newPageActive->save();
                }
            };
